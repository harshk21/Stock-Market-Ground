package com.smg.stock_market_ground.utils

object Constants {
    const val BASE_URL = "https://steamydowntown.backendless.app/api/1F3BD37D-C680-03E0-FFF0-70ADEE9F3E00/C40B514B-7D5D-4244-98FC-1E0BE150631D/"
    const val SPLASH_TIME: Long = 2500
    const val PASSWORD_REGEX = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\\S+$).{4,}$"
    const val TOKEN = "token"
    const val DATA = "user_data"
    const val WELCOME_CHECK = "isWelcome"
    const val IS_DISCOVER = "isDiscover"
    const val IS_SAVED = "isSaved"
    const val USER_CHECK = "isLoggedIn"
    const val SENT_MAIL = "Mail Sent Successfully"
}